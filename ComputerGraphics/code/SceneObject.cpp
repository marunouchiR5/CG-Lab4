...
void SceneObject::paintTexture(glm::vec3 isect, char r, char g, char b) {
	int width = blendTexture->getWidth();
	int height = blendTexture->getHeight();

	/* convert from xyz on the sphere to UV coordinate https://en.wikipedia.org/wiki/UV_mapping */
	/* note the negation of the y value because of UV origin is at upper left and xyz origin is at lower left*/
	/* also, the y value is multiplied by 2 such that the radius of the sphere is 1*/
	float u = 0.5 + (atan2(isect.x, isect.z) / (2.0 * PI));
	float v = 0.5 + (asin(-isect.y * 2.0f) / PI);
